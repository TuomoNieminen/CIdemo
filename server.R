...
shinyServer(function(input, output, session) {
  ...
  output$theoretical <- renderPlot({
    mu <- input$mean
    sd <- input$sd
    curve(dnorm(x, input$mean, input$sd), 
  ...
  observe({
    d$more_samples <- sapply(1:input$N, function(i) rnorm(d$n, d$mu, d$sd))
    ...
    d$z_prc_inside <- round(100*sum(d$z_contains_true)/input$N,1)
    
    d$t_contains_true <- contains_true(d$mu, d$t_CI[4:5,])
    d$t_prc_inside <- round(100*sum(d$t_contains_true)/input$N,1)
  ...
  output$howmany_zci <- renderUI({
    sliderInput("howmany_zci", "how many confidence intervals to plot?",
                min = 2, max = input$N, step = 2, value = 10)
  })
